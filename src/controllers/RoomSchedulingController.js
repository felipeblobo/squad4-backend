...
class RoomSchedulingController {
  static async listRoomScheduling(req, res) {
    try {
      ...
      return res
        .status(404)
        .json({
          mensagem: "Não foi possível listar os agendamentos para reuniões.",
        });
    }
  }
  ...
  static async getRoomBydId(req, res) {
    const { id } = req.params;
    try {
      const room = await database.RoomScheduling.findOne({
        where: { id: Number(id) },
      });
      ...
      res
        .status(404)
        .json({ mensagem: "Não foi possível obter este agendamento." });
    }
  }
  ...
  static async getRoomByUserId(req, res) {
    const { id } = req.params;
    try {
      const room = await database.RoomScheduling.findAll({
  ...
  static async registerOfficeScheduling(req, res) {
    ...
    const BusySchedule = await database.RoomScheduling.findAll({
      where: {
        [Op.and]: [{ date: dateScheduling }, { user_id: user }],
      },
    });
    ...
    if (BusySchedule.length > 0) {
      return res
        .status(400)
        .json({ mensagem: "O usuário já agendou para data solicitada." });
    }
    ...
    if (BusyRoom.length > 0) {
      return res
        .status(400)
        .json({ mensagem: "Esta sala já foi reservada para este horário." });
    }

    const schedulingOnCertainDate =
      await database.RoomScheduling.findAndCountAll({
        where: {
          [Op.and]: [{ date: dateScheduling }, { office: office }],
        },
      });
    ...
    if (schedulingOnCertainDate.count <= officeCapacity) {
      try {
        ...
        return res
          .status(500)
          .json({ mensagem: "Não foi possível realizar este agendamento." });
      }
    } else {
      return res
        .status(500)
        .json({
          mensagem:
            "Não há mais salas de reunião disponíveis para o dia solicitado.",
        });
    }
  }
  ...
  static async updateRoomScheduling(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ mensagem: "Não foi possível atualizar este agendamento." });
    }
  }
  ...
  static async deleteRoomScheduling(req, res) {
    ...
    try {
      ...
      return res
        .status(204)
        .json({ mensagem: `O agendamento com o id:${id} foi deletado.` });
    } catch (error) {
      return res
        .status(500)
        .json({ mensagem: "Não foi possível deletar este agendamento." });
    }
  }
  ...
  static async roomSchedulingByDate(req, res) {
    ...
    try {
      const schedulingOnCertainDate =
        await database.RoomScheduling.findAndCountAll({
          where: {
            [Op.and]: [{ date: data }, { time_zone: timezone }],
          },
        });
      ...
      return res
        .status(404)
        .json({
          messagem: "Não foi possível obter as reuniões para data solicitada.",
        });
    }
  }
}
